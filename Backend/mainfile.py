...
feature_order = None
try:
    # RandomForest pipeline (optional)
    rf_model = joblib.load("SavedModel.pkl")
    feature_order = joblib.load("featuresaved.pkl")
except Exception:
    # Keep RF optional; endpoint will check availability.
    rf_model = None
    try:
        # Attempt to load feature order even if RF model is missing
        feature_order = joblib.load("featuresaved.pkl")
    except Exception:
        feature_order = None

try:
    # Prophet components (required for forecast endpoints)
    prophet_model = joblib.load("prophet_model.pkl")
    prophet_scaler = joblib.load("prophet_scaler.pkl")
except Exception as e:
    # Prophet is required for forecast endpoints; raise on startup if missing
    raise RuntimeError("Failed loading Prophet models: " + str(e))
...
def predict_rf(data: CropInput):
    try:
        if rf_model is None or feature_order is None:
            raise HTTPException(status_code=503, detail="RandomForest model unavailable. Please provide SavedModel.pkl and featuresaved.pkl.")
        input_df = pd.DataFrame([data.dict()])
